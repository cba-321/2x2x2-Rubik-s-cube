...
// Checking for completeness of each face
...
// A struct for maintaining the Cube state
typedef struct CubeState {
    cube_t cube;
    char moves[50];             // Sequence of moves to reach this state
} CubeState;
...
void solvePocketCube(cube_t* initCube, int * count) {
    Queue queue;
    initQueue(&queue);
    int a =1;
    CubeState initState;
    initState.cube = *initCube;
    memset(initState.moves, 0, sizeof(initState.moves));

    enqueue(&queue, initState);
